        ...
        return LayoutBuilder(
          builder: (context, constraints) {
            ...
            if (forceSingleRow) {
              ...
              } else if (maxAvailableHeight != null &&
                  maxAvailableHeight!.isFinite) {
                boundedHeight = maxAvailableHeight!;
              }

              const double minTileHeight = 96;
              double resolvedTileHeight = preferredTileHeight;
              final int tileCount = metrics.length;
              if (boundedHeight != null && boundedHeight.isFinite && tileCount > 0) {
                final double totalSpacing = tileCount > 1 ? spacing * (tileCount - 1) : 0;
                final double heightForTiles = math.max(0, boundedHeight - totalSpacing);
                if (heightForTiles > 0) {
                  final double evenHeight = heightForTiles / tileCount;
                  resolvedTileHeight = evenHeight
                      .clamp(minTileHeight, preferredTileHeight)
                      .toDouble();
                }
              }

              final double widthScale = maxTileWidth > 0
                  ? (maxTileWidth / (screenWidth * 0.28)).clamp(0.55, 1.0).toDouble()
                  : 0.7;
              final double heightScale = (resolvedTileHeight / preferredTileHeight)
                  .clamp(0.55, 1.0)
                  .toDouble();
              final double visualScale = math.min(widthScale, heightScale);
